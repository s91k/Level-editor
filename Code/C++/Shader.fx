...
float3 calcFog(float3 _worldPos)
{
	const float START_DISTANCE = 300.0f; // The distance that determines when fog start to apply.
	const float END_DISTANCE = 600.0f; // The distance that determines when fog stop to apply.
	float distance = length(_worldPos); // Distance from camera to world position.

	float3 fog = float3(0.0f, 0.0f, 0.0f);
	if(distance >= END_DISTANCE) // Check if the distance is too far.
	{
		fog = float3(1.0f, 1.0f, 1.0f);
	}
	else if(distance >= START_DISTANCE) // Check if the distance is enough to calculate
	{
		float fogVal = (distance-START_DISTANCE)/(END_DISTANCE-START_DISTANCE);
		fog = float3(fogVal, fogVal, fogVal);
	}
...
{
	float3 fog = calcFog(mul(float4(input.WorldPos, 1.0f), g_mWorldView).xyz);

	//Check if the pixel is completely fog covered
	if(fog.x == 1.0f)
	{
	...
	{
		diffuse = float4(calcDiffuseLight(input.WorldPos, normal, diffuseColor), 1.0f);
	}
	...
	//return float4((ambient + diffuse.xyz + specular)*shadowCoeff+fog, diffuse.w/255.0f);
	return diffuse;
}
...
void BackgroundForwardGS( point GS_DRAW_BACKGROUND_IN input[1], inout TriangleStream<PS_DRAW_BACKGROUND_IN> triStream )
{
	...
	output.TexCoordFar = float2(input[0].Pos.x * 0.005 - input[0].Pos.y * 0.001, input[0].Pos.z * 0.005 + 0.3 + input[0].Pos.y * 0.001);
	output.TexCoordClose = float2(input[0].Pos.x * 0.01 - input[0].Pos.y * 0.005, input[0].Pos.z * 0.01 + 0.3 + input[0].Pos.y * 0.005);
	triStream.Append(output);
	
	output.Pos = float4(-1, -1, 0, 1);
	output.TexCoordFar = float2(input[0].Pos.x * 0.005 - input[0].Pos.y * 0.001, input[0].Pos.z * 0.005 - input[0].Pos.y * 0.001);
	output.TexCoordClose = float2(input[0].Pos.x * 0.01 - input[0].Pos.y * 0.005, input[0].Pos.z * 0.01 - input[0].Pos.y * 0.005);
	triStream.Append(output);

	output.Pos = float4(1, 1, 0, 1);
	output.TexCoordFar = float2(input[0].Pos.x * 0.005 + 0.53 + input[0].Pos.y * 0.001, input[0].Pos.z * 0.005 + 0.3 + input[0].Pos.y * 0.001);
	output.TexCoordClose = float2(input[0].Pos.x * 0.01 + 0.53 + input[0].Pos.y * 0.005, input[0].Pos.z * 0.01 + 0.3 + input[0].Pos.y * 0.005);
	triStream.Append(output);

	output.Pos = float4(1, -1, 0, 1);
	output.TexCoordFar = float2(input[0].Pos.x * 0.005 + 0.53 + input[0].Pos.y * 0.001, input[0].Pos.z * 0.005 - input[0].Pos.y * 0.001);
	output.TexCoordClose = float2(input[0].Pos.x * 0.01 + 0.53 + input[0].Pos.y * 0.005, input[0].Pos.z * 0.01 - input[0].Pos.y * 0.005);
	triStream.Append(output);
}
